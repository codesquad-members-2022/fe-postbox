...
- [x] JSON fetch
  - [x] 서버로부터 towns 배열을 JSON 으로 받아 render
...
- 변수 이름에 어느정도까지 정보를 담아야 하는지가 궁금합니다.
...
`townNodesHaveMailbox`를 정렬하면 상태가 바뀌게되는데,

```js
const descTownNodesHaveMailbox = townNodesHaveMailbox
  .slice()
  .sort(descByMailboxSize);
```

상태를 바꾸지 않고 위와 같이 새로운 변수에 저장해서 사용하는 것이 가독성이 좋은건지 모르겠습니다.
